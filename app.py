...
current_odds = st.session_state.current_perp_odds if st.session_state.use_perpetual and 'current_perp_odds' in st.session_state else None
if not st.session_state.use_perpetual and 'current_race' in locals():
    current_odds = current_race['odds']

if prev_race:
    implied = (1 / current_odds) * 100 if current_odds and current_odds > 0 else 0
    model = 60.0
    edge = model - implied
