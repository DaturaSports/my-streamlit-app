...
def calculate_stake():
    if st.session_state.just_resumed:
        st.session_state.just_resumed = False
        return st.session_state.base_stake
    return st.session_state.base_stake * (2 ** st.session_state.loss_streak)
    ...
    if st.button("❌ Loss", key="loss_btn", use_container_width=True):
        ...
        if current_stake <= st.session_state.bankroll:
            ...
            if st.session_state.bankroll > 0:
                ...
                st.info("Bankroll is \$0. Reset stake to continue.")
                if st.button("🔄 Reset Stake (Zero Bankroll)", key="reset_after_bust"):
                    st.session_state.loss_streak = 0
                    st.session_state.just_resumed = True
                    st.session_state.race_count += 1
                    st.rerun()
...
if current_stake > st.session_state.bankroll and st.session_state.bankroll > 0:
    st.warning("⚠️ Stake exceeds available bankroll. Action required below.")
elif st.session_state.bankroll == 0:
    st.info("Bankroll depleted. Use 'Reset Stake' to restart betting.")
...
if st.button("🔁 Reset Loss Streak"):
    st.session_state.loss_streak = 0
    st.session_state.just_resumed = True
    st.success("Loss streak reset. Next bet uses base stake.")
    st.rerun()
